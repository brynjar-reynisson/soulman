...
See `docs/superpowers/specs/2026-07-27-discord-do-not-disturb-design.md`. Only `notifybatch.Batcher`'s real-time notification path is gated — the once-daily digest cron (`scheduler.Scheduler`, default `10:00`) is untouched, still sending through the plain feign-wrapped `notifier`. `action-svc/dnd.WrapNotifier` mirrors `feign.WrapNotifier`'s shape exactly: during the configured window (default `00:00`-`10:00`, local time, `do_not_disturb.enabled`/`.start`/`.end` in `config/prod.json`), `Send` appends to `$SOULMAN_ROOT/logs/dnd-pending.txt` instead of sending; outside the window, it delegates straight through. A background `dnd.Flusher` wakes at the window's end time each day (same wake-loop mechanics as `Scheduler.loop`/`nextRun`), sends the accumulated pending content as one message — still through the feign-wrapped notifier, so `feign_mode` continues to govern real-vs-recorded — with up to 3 retries on send failure (same backoff as `Scheduler.sendWithRetry`). The pending file is cleared atomically with the read (inside the same lock), before the send is attempted — so a send failure never leaves content behind for a cross-day retry chain, and a message appended concurrently with a flush can never be silently wiped by a stale read.
